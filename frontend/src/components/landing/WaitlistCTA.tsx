...
export default function WaitlistCTA() {
  ...
  return (
    <section id="waitlist" className="bg-th-bg-primary py-[clamp(60px,8vw,120px)] px-5">
      <div className="max-w-[640px] mx-auto text-center">
        <p className="font-text text-[14px] font-semibold text-th-accent-blue tracking-[0.5px] uppercase mb-4">
          Early Access
        </p>
        <h2 className="text-section-heading text-th-text-primary mb-4">Be first. Grow faster.</h2>
        <p className="font-text text-[1.06rem] text-th-text-secondary leading-[1.47] tracking-[-0.374px] max-w-[480px] mx-auto mb-10">
          CreatorFlow is launching soon. Join the waitlist and get early access, exclusive founder pricing, and a free AI content audit of your channel.
        </p>
        ...
        {submitted ? (
          <div className="flex flex-col items-center gap-3 py-10 px-8 bg-th-feature-card-bg rounded-xl shadow-waitlist-success">
            ...
            <p className="font-text text-[15px] text-th-text-secondary leading-[1.47] max-w-[360px]">
              We&apos;ll email you at <strong className="text-th-text-primary">{email}</strong> when CreatorFlow is ready for you.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 max-w-[460px] mx-auto">
            <div className="flex gap-[10px] flex-wrap">
              <input
                ...
                required
                className={`flex-1 min-w-[220px] py-3 px-[18px] rounded-[980px] border-[1.5px] bg-th-feature-card-bg font-text text-[16px] text-th-text-primary outline-none transition-colors ${error ? 'border-th-error' : 'border-black/12 focus:border-th-accent-blue'}`}
              />
              <button
                type="submit"
                disabled={loading}
            ...
            {error && (
              <p className="font-text text-[13px] text-th-error text-center tracking-[-0.12px]">{error}</p>
            )}
        ...
        <div className="flex gap-6 justify-center flex-wrap mt-12">
          {['✓ Early access priority', '✓ Founder pricing', '✓ Free AI content audit'].map((item) => (
            <span key={item} className="font-text text-[14px] text-th-text-tertiary tracking-[-0.224px]">
              {item}
            </span>
          ))}
        </div>
