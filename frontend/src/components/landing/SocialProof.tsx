...
export default function SocialProof() {
  return (
    <section id="social-proof" className="bg-th-bg-primary py-[clamp(60px,8vw,120px)] px-5">
      <div className="max-w-[980px] mx-auto">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-[1px] bg-th-border rounded-xl overflow-hidden mb-20">
          {stats.map((stat) => (
            <div key={stat.label} className="bg-th-stat-bg py-10 px-6 text-center">
              ...
              <div className="font-text text-sm text-th-text-tertiary tracking-[-0.224px]">{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center mb-12">
          <h2 className="text-section-heading text-th-text-primary mb-3">Creators are already talking.</h2>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-5">
          {testimonials.map((t) => (
            <div key={t.name} className="bg-th-bg-card rounded-xl p-7 border border-th-border-card shadow-social-proof flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-display text-sm font-semibold text-th-text-primary tracking-[-0.224px]">{t.name}</div>
                  <div className="font-text text-xs text-th-text-tertiary tracking-[-0.12px]">{t.handle} · {t.subs}</div>
                </div>
