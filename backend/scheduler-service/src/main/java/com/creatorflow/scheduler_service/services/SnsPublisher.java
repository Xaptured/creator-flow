...
public class SnsPublisher {
    ...
    /**
     * Publish an event to the {@code creatorflow-events} SNS topic.
     *
     * @param event the event to publish
     * @return SNS message ID
     */
    public String publishToCreatorflowEvents(CreatorflowEventMessage event) {
        String topicArn = awsProperties.getTopics().get("creatorflow-events");
        if (topicArn == null || topicArn.isBlank()) {
            throw new IllegalStateException("SNS topic ARN not configured: app.aws.topics.creatorflow-events");
        }
        return publish(topicArn, event);
    }

    /**
     * Publish an event to any topic by ARN key from {@code app.aws.topics.*}.
     *
     * @param topicKey key in the topics map (e.g. "creatorflow-events")
     * @param event    the event to publish
     * @return SNS message ID
     */
    ...
    private String publish(String topicArn, CreatorflowEventMessage event) {
        String messageBody;
        try {
